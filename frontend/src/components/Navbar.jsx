...
const Navbar = () => {
  ...
  const { windowWidth, user, setUser, userInfo, setUserInfo } =
  ...
  const handleLogout = () => {
    axios
      .post(
        `https://airbnb-bdfq.onrender.com/logout`,
        {},
        {
    ...
    <>
      <div className="w-[90%] mx-auto">
        <div className="flex justify-between items-center mt-4 md:mt-5">
          ...
          {windowWidth >= 1024 && <SearchBox />}
          <div className="relative flex items-center gap-4">
            <div
              ref={searchButtonRef}
              onClick={() => setShowSearchBox(!showSearchBox)}
              className="flx cursor-pointer h-fit rounded-full bg-[#FF385C] p-3 md:hidden hover:bg-[#E00B41]"
            >
              <svg
            ...
            <div
              ...
              className="flex border cursor-pointer border-custom-gray gap-2.5 sm:gap-3 lg:gap-3.5 items-center p-2 h-min rounded-custom transition duration-200 hover:shadow-avatar-bs"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 32 32"
              ...
              {user ? (
                <Avatar name={userInfo.name} host={false} />
              ) : (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 32 32"
            ...
            {showLogin && (
              <div
                ref={loginRef}
                className="w-60 smd:w-72 rounded-lg bg-white absolute top-14 py-2 z-20 right-0 shadow-search-bs"
              >
                {!user && (
                  <>
                    <p onClick={() => navigate("/login")} className="links">
                      Log in
                    </p>
                    <p onClick={() => navigate("/register")} className="links">
                      Sign up
                    </p>
                    <div className="border-b py-1 mb-2"></div>
                  </>
                )}
                <p onClick={() => navigate("/create")} className="links">
                  Airbnb your home
                </p>
                {user && (
                  <>
                    <p
                      onClick={() => navigate(`/${userInfo._id}/wishlist`)}
                      className="links"
                    >
                      Wishlists
                    </p>
                    <div className="border-b py-1 mb-2"></div>
                    <p
                      onClick={() => navigate(`/${userInfo._id}/trips`)}
                      className="links"
                    >
                      Trips
                    </p>
                    <p
                      onClick={() => navigate(`/${userInfo._id}/listings`)}
                      className="links"
                    >
                      My Listings
                    </p>
                    <div className="border-b py-1 mb-2"></div>
                    <p onClick={handleLogout} className="links">
                      Logout
                    </p>
                  </>
                )}
        ...
        {windowWidth < 1024 && (
          <div ref={searchBoxRef} className="mt-4 flex justify-center">
            <SearchBox showSearchBox={showSearchBox} />
          </div>
        )}
      </div>
      <div
        className={`left-0 right-0 border-b-2${
          showSearchBox ? "py-2.5" : ""
        } md:py-2.5`}
      ></div>
    </>
  );
